...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  // Menu items para cajeros (cashier) - Sprint 3
  const cashierMenuItems = [
    { path: '/payments', icon: CreditCard, label: 'Gestión de Pagos' },
    { path: '/sales-history', icon: ShoppingCart, label: 'Historial de Ventas' },
      ...
      `}>
        ...
        <nav className="flex-1 mt-6 overflow-y-auto">
          <div className="px-3 space-y-1">
            ...
            {/* Menu items para cajeros - Sprint 4 */}
                ...
                {cashierMenuItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`group flex items-center px-3 py-3 text-sm font-medium rounded-lg transition-all duration-200 ${
                        isActive(item.path)
                          ? 'bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-800 shadow-md border-l-4 border-blue-500'
                          : 'text-gray-600 hover:bg-gradient-to-r hover:from-blue-50 hover:to-cyan-50 hover:text-blue-700'
                      }`}
                    >
                      <Icon className={`mr-3 h-5 w-5 flex-shrink-0 ${
