...
import { BarChart3, TrendingUp, DollarSign, CreditCard, Calendar, Filter, MapPin, Download } from 'lucide-react';
...
export default function ReportsByVenue() {
  ...
  const loadLocations = async () => {
    try {
      ...
      // Cashiers only see their assigned venues
  ...
  const getLocationName = () => {
    ...
    return location ? `${location.name} (${location.code})` : 'Selected Venue';
  };
    ...
    <div className="container mx-auto px-4 py-4 sm:py-6 md:py-8">
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Venue Reports</h1>
            <p className="text-sm sm:text-base text-gray-600">
              Sales reports and analytics for your venue
            </p>
          </div>
          {report && (
            <button
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-4 sm:p-6 text-white">
            <h2 className="text-xl sm:text-2xl font-bold mb-2">{getLocationName()}</h2>
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
              ...
              <div className="space-y-3">
                {report.by_payment_method.map((method) => (
                  <div key={method.payment_method} className="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm sm:text-base font-medium text-gray-900 capitalize">{method.payment_method}</span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs sm:text-sm text-gray-600 w-12 sm:w-16 text-right">{method.percentage.toFixed(1)}%</span>
                      </div>
                      <p className="text-xs sm:text-sm text-gray-600 mt-1">{method.total_orders} orders</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {report.summary.total_orders === 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
              <p className="text-gray-700">No sales data found for the selected venue and date range</p>
            </div>
          )}
        </div>
      ) : (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
          ...
          <p className="text-gray-600">Select a venue and generate a report to view analytics</p>
        </div>
      )}
    </div>
  );
